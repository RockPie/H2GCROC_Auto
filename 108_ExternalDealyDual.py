...
script_id_str       = '108_ExternalDelayDual'
...
h2gcroc_ip      = "10.1.2.208" # ! Overriding the IP address
...
L1_offset_scan_range    = range(5, 6, 1)
...
try:
    ...
    for _chn in range(152):
        ...
        if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip, h2gcroc_port, asic_num=_asic_num, fpga_addr = fpga_address, sub_addr=_sub_addr, reg_addr=0x00, data=_chn_content, retry=3, verbose=i2c_setting_verbose):
            logger.warning(f"Failed to set ChannelWise settings for board 0, ASIC {_asic_num}")
        if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic_num, fpga_addr = fpga_address_second, sub_addr=_sub_addr, reg_addr=0x00, data=_chn_content, retry=3, verbose=i2c_setting_verbose):
            logger.warning(f"Failed to set ChannelWise settings for board 1, ASIC {_asic_num}")
    ...
    for _asic_num in range(total_asic):
        ...
        if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic_num, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Digital_Half_0"], reg_addr=0x00, data=default_digital_half, retry=3, verbose=i2c_setting_verbose):
            logger.warning(f"Failed to set Digital_Half_0 settings for Board 1 ASIC {_asic_num}")
        if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic_num, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Digital_Half_1"], reg_addr=0x00, data=default_digital_half, retry=3, verbose=i2c_setting_verbose):
            logger.warning(f"Failed to set Digital_Half_1 settings for Board 1 ASIC {_asic_num}")
    ...
    for _asic_num in range(total_asic):
        ...
        if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic_num, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Global_Analog_0"], reg_addr=0x00, data=default_global_analog, retry=3, verbose=i2c_setting_verbose):
            logger.warning(f"Failed to set Global_Analog_0 settings for Board 1 ASIC {_asic_num}")
        if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic_num, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Global_Analog_1"], reg_addr=0x00, data=default_global_analog, retry=3, verbose=i2c_setting_verbose):
            logger.warning(f"Failed to set Global_Analog_1 settings for Board 1 ASIC {_asic_num}")
    ...
    for _asic_num in range(total_asic):
        ...
        if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic_num, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Reference_Voltage_0"], reg_addr=0x00, data=_ref_content_half0, retry=3, verbose=i2c_setting_verbose):
            logger.warning(f"Failed to set Reference_Voltage_0 settings for Board 1 ASIC {_asic_num}")
        if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic_num, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Reference_Voltage_1"], reg_addr=0x00, data=_ref_content_half1, retry=3, verbose=i2c_setting_verbose):
            logger.warning(f"Failed to set Reference_Voltage_1 settings for Board 1 ASIC {_asic_num}")

    # * --- Set up HV ---------------------------------------------------------
    logger.info("Setting up HV")
    socket_udp.sendto(bytes(HV_on), (h2gcroc_ip, h2gcroc_port))
    time.sleep(0.1)
    socket_udp.sendto(bytes(HV_on_second), (h2gcroc_ip_second, h2gcroc_port))
    ...
    # for _ex_trg_val in progress_bar:
        ...
        with open(output_data_txt_path, 'w') as data_file:
            ...
            for _asic in range(total_asic):
                ...
                if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Digital_Half_0"], reg_addr=0x00, data=_digital_content, retry=3, verbose=i2c_setting_verbose):
                    logger.warning(f"Failed to set L1 delay value {_L1_delay_value} for ASIC {_asic} for half 0")
                if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Digital_Half_1"], reg_addr=0x00, data=_digital_content, retry=3, verbose=i2c_setting_verbose):
                    logger.warning(f"Failed to set L1 delay value {_L1_delay_value} for ASIC {_asic} for half 1")
            ...
            if not packetlib.send_check_DAQ_gen_params(socket_udp, h2gcroc_ip_second, h2gcroc_port, 0, fpga_addr=fpga_address_second, data_coll_en=0x03, trig_coll_en=0x00, daq_fcmd=75, gen_preimp_en=0, gen_pre_interval = gen_pre_inverval_value, gen_nr_of_cycle=gen_nr_cycle, gen_pre_fcmd=gen_fcmd_internal_injection,ext_trg_en=1, ext_trg_delay=_ex_trg_val, ext_trg_deadtime=255, gen_fcmd=gen_fcmd_L1A,gen_interval=gen_interval_value, daq_push_fcmd=75, machine_gun=machine_gun_val,verbose=False):
                logger.warning(f"Failed to set up the generator for L1 delay value {_L1_delay_value}")
            ...
            # logger.debug(f"Received data: {unpacked_data}")
            ...
            for _asic in range(total_asic):
                ...
                if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Top"], reg_addr=0x00, data=top_reg_runLR, retry=3, verbose=i2c_setting_verbose):
                    logger.warning(f"Failed to set Top settings RunLR for Board 1 ASIC {_asic} for run")
            ...
            if not packetlib.send_daq_gen_start_stop(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=0, fpga_addr = fpga_address_second, daq_push=0x00, gen_start_stop=0, daq_start_stop=0xFF, verbose=False):
                logger.warning(f"Failed to start generator for Board 1 L1 delay value {_L1_delay_value}")
            ...
            while True:
                try:
                    data_packet, rec_addr    = socket_udp.recvfrom(16384)
                    ...
                    if _line_counter % 2000 == 0:
                        logger.info(f"Current event number: {_line_counter//8}")
                    if _line_counter >= expected_event_num*8:
                        break
                    # do nothing and just take data
            ...
            if not packetlib.send_daq_gen_start_stop(socket_udp, h2gcroc_ip, h2gcroc_port, asic_num=0, fpga_addr = fpga_address, daq_push=0x00, gen_start_stop=0, daq_start_stop=0x00, verbose=False):
                logger.warning(f"Failed to start generator for L1 delay value {_L1_delay_value}")
            if not packetlib.send_daq_gen_start_stop(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=0, fpga_addr = fpga_address_second, daq_push=0x00, gen_start_stop=0, daq_start_stop=0x00, verbose=False):
                logger.warning(f"Failed to start generator for Board 1 L1 delay value {_L1_delay_value}")
            ...
            for _asic in range(total_asic):
                ...
                if not packetlib.send_check_i2c_wrapper(socket_udp, h2gcroc_ip_second, h2gcroc_port, asic_num=_asic, fpga_addr = fpga_address_second, sub_addr=packetlib.subblock_address_dict["Top"], reg_addr=0x00, data=top_reg_offLR, retry=3, verbose=i2c_setting_verbose):
                    logger.warning(f"Failed to set Top settings OffLR for Board 1 ASIC {_asic} for run")
